...
const socialIcons: Record<string, React.ReactNode> = {
  ...
  line: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="h-[18px] w-[18px]">
      <path d="M19.365 9.863c.349 0 .63.285.63.631 0 .348-.281.63-.63.63H17.61v1.125h1.755c.349 0 .63.283.63.63 0 .349-.281.631-.63.631h-2.386c-.345 0-.627-.282-.627-.63V8.108c0-.348.282-.63.63-.63h2.386c.346 0 .627.282.627.63 0 .349-.281.63-.63.63H17.61v1.125h1.755zm-3.855 3.016c0 .27-.174.51-.432.596-.064.021-.133.031-.199.031-.211 0-.391-.09-.51-.25l-2.443-3.317v2.94c0 .344-.279.627-.631.627-.346 0-.626-.283-.626-.628V8.108c0-.27.173-.51.43-.595.06-.023.136-.033.194-.033.195 0 .375.105.495.254l2.462 3.33V8.108c0-.345.282-.63.63-.63.345 0 .63.285.63.63v4.771zm-5.741 0c0 .344-.282.627-.631.627-.345 0-.627-.283-.627-.627V8.108c0-.348.282-.63.63-.63.346 0 .628.282.628.63v4.771zm-2.466.627H4.917c-.345 0-.63-.282-.63-.627V8.108c0-.348.285-.63.63-.63.348 0 .63.282.63.63v4.141h1.756c.348 0 .629.283.629.63 0 .349-.281.631-.629.631M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314"/>
    </svg>
  ),
  instagram: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="h-[18px] w-[18px]">
...
function getSocialColor(platform: string) {
  const key = platform.toLowerCase().trim();
  return socialColors[key] || 'var(--accent)';
}
  ...
  return (
    <footer className="mt-20 sm:mt-28 border-t border-[var(--border)] bg-[var(--surface)]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {/* Top */}
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-3 py-12 sm:py-16">
          {/* Brand column */}
          ...
          <div>
            <h4 className="text-[0.85rem] font-semibold text-[var(--text)] mb-4">{t('footer.contact_us')}</h4>
            <div className="flex items-center gap-3 flex-wrap">
              {socialLinks.length > 0 ? (
                socialLinks.map((link, i) => {
                  ...
                  return (
                    <a
                      ...
                      title={link.platform}
                      className="group/social relative flex h-10 w-10 items-center justify-center rounded-xl border border-[var(--border)] bg-[var(--surface-alt)] text-[var(--muted)] transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
                      style={{
                        // @ts-ignore
                        '--social-color': color,
                ...
                <p className="text-[0.8rem] text-[var(--muted-soft)] italic">
                  {t('footer.no_social')}
                </p>
              )}
            </div>
